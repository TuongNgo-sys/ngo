...
# Load dữ liệu cây trồng, lịch sử cảm biến, cấu hình
...
# -----------------------
# Cấu hình hệ thống: khung giờ tưới & chế độ
# -----------------------
...
# -----------------------
# Kiểm tra khung giờ tưới hiện tại
# -----------------------
...
# -----------------------
# MQTT Client để nhận dữ liệu ESP32
# -----------------------
...
# Dữ liệu cảm biến mới nhất (toàn bộ app dùng)
...
def on_message(client, userdata, msg):
    ...
    # Khi nhận đủ 2 dữ liệu, thêm vào lịch sử
    if sensor_data["soil_humidity"] is not None and sensor_data["soil_temperature"] is not None:
        ...
        # Reset sensor_data để không lưu trùng
...
# -----------------------
# Hiển thị dữ liệu cảm biến thực tế
# -----------------------
...
# -----------------------
# Quyết định tưới dựa trên dữ liệu thực tế (Auto)
# -----------------------
...
if mode == "auto":
    ...
    pump_status = False  # không tự bật bơm
...
st.markdown("---")
st.info(_(
    "🌟 Hệ thống tự động nhận dữ liệu độ ẩm và nhiệt độ đất từ ESP32 qua MQTT.\n"
    "🌟 Chế độ tự động sẽ tự bật bơm khi độ ẩm thấp hơn chuẩn và trong khung giờ tưới.\n"
    "🌟 Chế độ thủ công không tự bật bơm, chỉ hiển thị trạng thái.",
    "🌟 The system automatically receives soil moisture and temperature data from ESP32 via MQTT.\n"
    "🌟 Auto mode will turn pump on when moisture is below threshold and during watering window.\n"
    "🌟 Manual mode does not auto control pump, only shows status."
))
